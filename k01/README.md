...
リニアサーチの関数とバイナリーサーチの関数を用いてIDから地域を検索するソースコード。
リニアサーチでは、69,70,71で配列の端からkeyと比較していき、バイナリーサーチでは、87行目から105行目は配列の真ん中から検索し、比較したIDより大きいか小さいかで右に検索していくか左かを決める。90行目からは真ん中よりkeyが大きかった場合、98行目からは真ん中よりkeyが小さかった時、と場合分けされている。85行目の式は真ん中を決めるための式である。
